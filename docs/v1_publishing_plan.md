...
- Keep `trail-docs` core local-first, deterministic, and citation-backed.
...
- Add `trail-docs fetch <selector>` where selector can be a discovered candidate, package spec, repo URL, or docs URL.
...
- Add `trail-docs prep <query_or_url> [--version]`:
  - discover -> fetch -> build -> emit manifest.
- Keeps existing `trail-docs use` experience unchanged after prep.
...
1. `trail-docs discover <query> [--ecosystem <name>] [--max-results <n>] [--json]`
2. `trail-docs fetch <selector> [--version <v>] [--ref <sha|tag>] [--out <dir>] [--json]`
3. `trail-docs prep <query_or_url> [--version <v>] [--path <dir>] [--json]` (P1 but recommended)

## Data Contract Additions
1. Source manifest (`.trail-docs/source.json`)
...
- Add policy file support (`trail-docs.policy.json`) and safe defaults.
