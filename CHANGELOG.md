...
- Project-level `doc-nav.toml` defaults (library/index/manifest/trust/federation/output).
- Source provenance manifest (`.doc-nav/source.json`) including canonical URL, requested/resolved refs, integrity, and trust signals.
- Optional source provenance in `cite` and `use` (`citation_details`).
- Security policy support via `doc-nav.policy.json` with host, extension, and size/file limits.
...
- Surface extraction now caches parsed artifacts under `.doc-nav/cache/surfaces`.
...
### Added
- Initial release of `doc-nav` with `bootstrap`, `build`, `list`, `stats`, `search`, `open`, `cite`, and `use`.
